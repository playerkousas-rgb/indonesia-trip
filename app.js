...
function formatDateRange(start, end) {
  if (!start || !end) return '2026/7/11 → 2026/7/20';
  ...
  return `
    <div class="list-item">
      ...
      <table class="table" style="margin-top:10px">
        ...
        <tr><td>護照到期日</td><td>${formatDisplayDate(p.passport_expiry) || '-'}</td></tr>
...
function renderGenericTable(rows) {
  ...
  return `<table class="table"><thead><tr>${headers.map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${rows.map(r=>`<tr>${headers.map(h=>`<td>${formatCellByKey(h, r[h])}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
}

function renderMembersTabbed(rows) {
  if (!rows.length) return '<div class="small">暫無資料</div>';
  const tabs = rows.map((r, i) => `<button class="btn btn-light member-tab ${i===0?'active':''}" data-member-tab="${i}">${r.chinese_name || r.display_name || '成員'}</button>`).join('');
  const panels = rows.map((r, i) => `
    <div class="member-panel ${i===0?'':'hidden'}" data-member-panel="${i}">
      <div class="list-item">
        <strong>${r.chinese_name || ''} ${r.english_name ? ' / ' + r.english_name : ''}</strong>
        <div class="meta"><span class="chip">${r.role_type || ''}</span><span class="chip">${r.scout_role || ''}</span></div>
        <table class="table" style="margin-top:10px">
          <tr><td>電話</td><td>${formatCell(r.phone)}</td></tr>
          <tr><td>Email</td><td>${formatCell(r.email)}</td></tr>
          <tr><td>緊急聯絡人</td><td>${formatCell(r.parent_name)} ${r.parent_relation ? '（' + r.parent_relation + '）' : ''}</td></tr>
          <tr><td>緊急聯絡電話</td><td>${formatCell(r.parent_phone)}</td></tr>
          <tr><td>護照號碼</td><td>${formatCell(r.passport_no)}</td></tr>
          <tr><td>護照到期日</td><td>${formatCellByKey('passport_expiry', r.passport_expiry)}</td></tr>
          <tr><td>健康備註</td><td>${formatCell(r.medical_notes)}</td></tr>
        </table>
      </div>
    </div>`).join('');
...
function bindMemberTabs() {
  document.querySelectorAll('[data-member-tab]').forEach(btn => btn.addEventListener('click', () => {
    document.querySelectorAll('[data-member-tab]').forEach(b => b.classList.remove('active'));
    document.querySelectorAll('[data-member-panel]').forEach(p => p.classList.add('hidden'));
    btn.classList.add('active');
    const idx = btn.dataset.memberTab;
    document.querySelector(`[data-member-panel="${idx}"]`)?.classList.remove('hidden');
  }));
}
...
function renderGroupedByCity(rows, cardId) {
  ...
  if (cardId === 'phrases') {
    return `<div class="list">${rows.map(r => `
      <div class="list-item">
        <strong>${formatCell(r.chinese)}</strong>
        <div class="small" style="margin-top:6px">英文：${formatCell(r.english)}</div>
        <div class="small">印尼文：${formatCell(r.indonesian)}</div>
  ...
  if (cardId === 'apps') {
    return `<div class="list">${rows.map(r => `
      <div class="list-item">
        ...
        <div class="small" style="margin-top:6px">用途：${formatCell(r.purpose)}</div>
        <div class="small">平台：${formatCell(r.platform)}</div>
        ${r.note ? `<div class="small">備註：${formatCell(r.note)}</div>` : ''}
        ${r.download_url ? `<div class="small" style="margin-top:8px">${formatCell(r.download_url)}</div>` : ''}
      </div>`).join('')}</div>`;
  }
  const groups = {};
  ...
  return Object.entries(groups).map(([city,list]) => `
    <div class="list-item">
      <strong>${city}</strong>
      <div class="list" style="margin-top:10px">${list.map(r => `<div class="list-item"><strong>${r.name || r.item_name || r.species_name || '-'}</strong>${Object.entries(r).filter(([k,v]) => !['city','name','item_name','species_name'].includes(k) && v).map(([k,v]) => `<div class="small" style="margin-top:6px">${labelize(k)}：${formatCellByKey(k, v)}</div>`).join('')}</div>`).join('')}</div>
    </div>`).join('');
}
...
function bindPackingTabs() {
  ...
  document.querySelectorAll('[data-packing-tab]').forEach(btn => btn.addEventListener('click', () => {
    ...
    document.querySelectorAll('[data-packing-panel]').forEach(p => p.classList.add('hidden'));
    btn.classList.add('active');
    const idx = btn.dataset.packingTab;
    document.querySelector(`[data-packing-panel="${idx}"]`)?.classList.remove('hidden');
  }));
}
...
function renderWeather(rows) {
  const cards = rows.map(r => `
    <div class="list-item">
      ...
      <div class="small" style="margin-top:6px">現時：${r.current_temp ?? '-'}°C｜體感：${r.apparent_temp ?? '-'}°C｜風速：${r.wind_speed ?? '-'} km/h</div>
      <div class="small">今日：${r.temp_min ?? '-'}°C - ${r.temp_max ?? '-'}°C｜降雨：${r.precipitation ?? 0} mm</div>
      <div class="small">明日：${r.tomorrow_min ?? '-'}°C - ${r.tomorrow_max ?? '-'}°C</div>
      <div class="small">日出：${formatTimeOnly(r.sunrise)}｜日落：${formatTimeOnly(r.sunset)}</div>
      ${r.sea_temp !== '' && r.sea_temp != null ? `<div class="small">海水溫度：${r.sea_temp}°C</div>` : ''}
      <div class="small">更新：${formatDateTime(r.time)}</div>
    </div>`).join('');
  return `<div class="list">${cards}<div class="list-item color-blue"><strong>資料來源</strong><div class="small">Open-Meteo forecast API；海水溫度只顯示峇里。</div></div></div>`;
}

function renderRates(rows) {
  return `<div class="list">${rows.map(r => `<div class="list-item"><strong>${r.pair}</strong><div style="margin-top:6px">${r.rate}</div><div class="small">更新：${formatCellByKey('updated_at', r.updated_at)}</div></div>`).join('')}</div>`;
}
...
function renderTransport(rows) {
  ...
  return `<div class="list">${rows.map(r => `
    <div class="list-item">
      <strong>${formatCell(r.route)}</strong>
      <div class="small" style="margin-top:6px">由：${formatCell(r.from_place)}</div>
      <div class="small">到：${formatCell(r.to_place)}</div>
      <div class="small">方式：${formatCell(r.method)}</div>
      <div class="small">車程：約 ${formatCell(r.estimated_time)}</div>
      <div class="small">費用：約 ${formatCell(r.estimated_cost)}</div>
      ${r.note ? `<div class="small">備註：${formatCell(r.note)}</div>` : ''}
    </div>`).join('')}</div>`;
}
...
function formatCell(v) {
  if (v == null || v === '') return '-';
  if (typeof v === 'string' && /^https?:\/\//.test(v)) return `<a class="link" href="${v}" target="_blank">開啟連結</a>`;
  return String(v).replace(/\n/g, '<br>');
}

function formatCellByKey(key, value) {
  if (value == null || value === '') return '-';
  const k = String(key || '').toLowerCase();
  if (k.includes('date') || k.includes('expiry')) return formatDisplayDate(value);
  if (k === 'updated_at' || k === 'time' || k.includes('sunrise') || k.includes('sunset')) return formatDateTime(value);
...
function formatDateTime(v) {
  if (v == null || v === '') return '-';
  const s = String(v).trim();
  if (/^\d{4}-\d{2}-\d{2}t/i.test(s) || /^\d{4}-\d{2}-\d{2}/.test(s)) {
    const d = new Date(s);
    if (!isNaN(d)) return `${d.getFullYear()}/${d.getMonth()+1}/${d.getDate()} ${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`;
  }
  return s;
}
...
async function renderAdmin() {
  ...
  if (state.adminTab === 'users') {
    ...
    wrap.innerHTML = `<div class="grid-2"><div class="list"><div class="list-item color-blue"><strong>CONFIG / CARDS / USER_CARD_ACCESS</strong><div class="small">系統設定型</div></div><div class="list-item color-red"><strong>USER / MEMBERS</strong><div class="small">敏感資料型</div></div></div><div class="list"><div class="list-item color-yellow"><strong>內容型工作表</strong><div class="small">itinerary / flights / hotels / notes / packing_list / weather_locations / transport_info / team_rules 等</div></div></div></div>`;
  }
}
